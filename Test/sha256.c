...
#define EP_0(x) (ROTRIGHT(x,2) ^ ROTRIGHT(x,13) ^ ROTRIGHT(x,22))
#define EP_1(x) (ROTRIGHT(x,6) ^ ROTRIGHT(x,11) ^ ROTRIGHT(x,25))
#define SIG_0(x) (ROTRIGHT(x,7) ^ ROTRIGHT(x,18) ^ ((x) >> 3))
#define SIG_1(x) (ROTRIGHT(x,17) ^ ROTRIGHT(x,19) ^ ((x) >> 10))

static const INT constant[64] = {
...
void sha256_transform(SHA256_Context *context, const CHAR data[])
{
	...
	for ( ; i < 64; ++i)
		m[i] = SIG_1(m[i - 2]) + m[i - 7] + SIG_0(m[i - 15]) + m[i - 16];
	...
	for (i = 0; i < 64; ++i) {
		t1 = h + EP_1(e) + CH(e,f,g) + constant[i] + m[i];
		t2 = EP_0(a) + MAJ(a,b,c);
		h = g;
		g = f;
		f = e;
...
void sha256_final(SHA256_Context *context, CHAR hash[])
{
	...
	for (i = 0; i < 4; ++i) {
		...
		hash[i + 20] = (context->Hash[5] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 24] = (context->Hash[6] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 28] = (context->Hash[7] >> (24 - i * 8)) & 0x000000ff;
	}
}
